...
def main():
    ...
    # P3-5 (已由 utils.py 統一): 這裡保留本地版本供獨立執行
    def emotion_score_local(content):
        """計算單篇貼文的情緒強度 (0-100)"""
        score = 0
        text = content
    ...
    emotions = []
    for p in originals:
        score = emotion_score_local(p['content'])
        emotions.append({
    ...
    with open(DATA / 'results_05_anomaly.json', 'w', encoding='utf-8') as f:
