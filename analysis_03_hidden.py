...
def main():
    ...
    for np_item in number_posts:
        all_ints.extend(int(v) for v in np_item['numbers'] if v == int(v))
    ...
    for e in top_excl[:5]:
        # 找到對應的貼文
        # P4-1: next() 加 default=None
        post = next((p for p in originals if p['created_at'][:10] == e['date']), None)
        if post:
    ...
    with open(DATA / 'results_03_hidden.json', 'w', encoding='utf-8') as f:
